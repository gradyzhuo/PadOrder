...
- (void) alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    CGRect frame = self.remindedView.frame;
    ...
    switch (buttonIndex) {
        case 0:
            ...
            [self changeTableViewAndRemindViewFrameByType:1];
...
- (void)changeTableViewAndRemindViewFrameByType:(NSInteger)type{
    CGRect frame = self.remindedView.frame;
    CGFloat x = frame.origin.x;
    CGFloat width = frame.size.width;
    ...
    switch (type) {
        case 0:
            frame = CGRectMake(x, 61, width, 518);
            break;
        case 1:
            frame = CGRectMake(x, 124, width, 455);
            break;
    }
    
    self.remindedView.frame = frame;
...
- (BOOL) checkHasFetched{
    ...
    if(count==0){
        ...
        [self changeTableViewAndRemindViewFrameByType:0];
    }
    else {
        self.navigationItem.rightBarButtonItem = nil;
